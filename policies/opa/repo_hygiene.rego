# repo_hygiene - repository hygiene policy for Ansible-framework repos.
#
# Encodes repository-level invariants visible from source files: workflow
# `uses:` SHA-pinning, privileged-trigger boundaries, and Ansible config
# defaults that keep the reference framework conservative.
...
deny contains msg if {
	has_ansible_cfg
	record := uncommented_line_records("ansible.cfg")[_]
	lower(record.text) == "host_key_checking = false"
	msg := "ansible.cfg must not disable host key checking"
}

deny contains msg if {
	has_ansible_cfg
	record := uncommented_line_records("ansible.cfg")[_]
	lower(record.text) == "retry_files_enabled = true"
	msg := "ansible.cfg must keep retry files disabled"
}
