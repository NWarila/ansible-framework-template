...
allowed_destination() {
  local value="${1%/}"
  case "${value}" in
    ansible/files|ansible/files/*|ansible/templates|ansible/templates/*|ansible/vars|ansible/vars/*|ansible/inventory|ansible/inventory/*|ansible/fixtures/runtime|ansible/fixtures/runtime/*)
      return 0
      ;;
    *)
      return 1
...
while IFS= read -r line; do
  ...
  if ! allowed_destination "${dst}"; then
    die "overlay destination must be under ansible/files, ansible/templates, ansible/vars, ansible/inventory, or ansible/fixtures/runtime: ${dst}"
  fi
